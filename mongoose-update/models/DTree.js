 ...
 * 12/4 获得最新结构 testjson/数据库结构说明.json
 * 由此获得schema
 * 12/9 完美食品适配 20141205_new_structure.json
 */
...
var dtreeSchema = new mongoose.Schema({
	config: { //config,用于存储决策树的一般配置信息
		...
		last_saved_time: { type: Date, default: Date.now}, //最近一次修改时间

		security_level: { type: String }, //安全等级，有私有，保护，完全公开三种模式
		layout_style: { type: String }, //决策树的主题样式
		layer_width: [{type: Number}], //决策树每一层的宽度(单位px)，这个数组的长度即树的深度
		...
		show_tip: {type: Boolean} //是否显示每一个节点的小贴士
	},
	//序列化反序列化的对象
	node_array: [{ //node_array， 节点信息，以对象数组方式存放
		...
		//markov_info: {type: String}, //存马尔科夫信息的，暂时无用
		...
		tracker_array: [{type: String}], //节点追踪值
		payoff_array: [{type: String}], //节点收益
		children:[]
	}],
	param_array:  [{ //param_array，变量，以对象数组方式存放
		...
		formula_pre: {type: String}, //Math.js看得懂的的计算表达式
